...
void Andor_Camera::initializeController()
{
 ...
 if (m_camera_number < numCameras && m_camera_number >= 0) {
  ...
  if (m_detector_model != "SIMCAM CMOS") {
                ...
                std::cout<< "Camera is ready: type " << m_detector_type << ", model " << m_detector_model;
                ...
                m_detector_type = std::string("Simulator");
                       ...
                       std::cout<< "The camera is indeed the SIMULATED camera, all exception for invalid parameter name will be ignored!!!";
...
//	FrameDim      the_frame_mem_dim;
	...
	std::cout << "BE VERY CAREFULL : The andor SDK3 camera that you are connected to is a SIMULATED CAMERA !!!";
 	...
 	std::cout << "Flushing the queue of the framegrabber";
 ...
 //	setInt(FrameCount, 100);
    ...
    std::cout << "Tried to set the acq is Fixed mode " << WStringToString(std::wstring(the_string));
...
void Andor_Camera::startAcq()
{
	...
	//	logStream(MESSAGE_DEBUG) << "Starting the acquisition by the camera (or triggering when in software trigger mode)"<<sendLog;
		...
		//sendCommand(AcquisitionStart);
			std::cout << "Acquisition start command run";
	}
...
// unpackedBuffer= (unsigned short*)Buffer2;
	...
	std::cout << "after queue buffer image index is"<<(long long)m_image_index;

        getBool(CameraAcquiring,&cameraIsAcquiring);
       }//while acquiring

		std::cout << "Done, the acquisition is now started and the frame retrieving should take place in parallel in a second thread";
		getBool(CameraAcquiring,&cameraIsAcquiring);
		std::cout  << cameraIsAcquiring;
		stopAcq();
		getBool(CameraAcquiring,&cameraIsAcquiring);
		std::cout  <<"after stopAcq"<< cameraIsAcquiring;
}
...
std::cout<< "getIsAcquiring value is " << isAcquiring ;
...
void Andor_Camera::setTemperatureSP(double temp)
{
	...
	if (propImplemented(TargetSensorTemperature) && m_detector_model.find("ZYLA5.5") == std::string::npos) {
		...
		if ((m_temperature_sp - temp) > 0.1 || (m_temperature_sp - temp) < -0.1) {
				    ...
				    std::cout<< "Proof-reading temperature set-point : "
					<< "\n\tproof-read = " << m_temperature_sp
					<< "\n\twhile asked to be = " << temp
				     << "Failed on setting temperature set-point";
...
void Andor_Camera::setAdcGain(A3_Gain iGain)
{
	...
	if (m_real_camera) {
		...
		if (m_adc_gain != iGain) {
				std::cout <<  "Proof-reading the ADC readout gain :"
					<< "\n\tGot " << m_adc_gain << " back,"
...
int Andor_Camera::getHwBitDepth(int *bit_depth)
{
	...
	//logStream(MESSAGE_DEBUG) << "Get BitDepth not implemented for simulated camera: "<< "fixing b16"<<sendLog;
	*bit_depth = b16;
	return AT_SUCCESS;
}
